...
rm(list = ls())
library(readxl)
kemiskinan <- read_excel("~/Semester 3/Metode Regresi/data_kemiskinan.xlsx")
library(readxl)
kemiskinan_unique <- read_excel("~/Semester 3/Metode Regresi/data_kemiskinan.xlsx")
View(kemiskinan_unique)
View(kemiskinan)
...
attr(kemiskinan_unique,'Kemiskinan') <- c("Kemiskinan", "Katmiskin")
attr(kemiskinan_unique,'IPM') <- c("IPM", "PLN")
str(attributes(kemiskinan_unique)) 
```
```{r}
(SepalVar <- attr(kemiskinan_unique, "Kemiskinan")) #Tentukan kelompok variabel berdasarkan atribut
```
```{r}
(PetalVar <- attr(kemiskinan_unique, "IPM"))
```
```{r}
# Selanjutnya, lihat korelasi dalam menggunakan ggpairs.

ggpairs(kemiskinan_unique, title = "seluruh data")
