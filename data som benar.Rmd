...
rm(list = ls())
library(readxl)
Golf <- read_excel("~/Semester 3/Data Mining/dataset/Golf.xls")
View(Golf)
```
#### Palet
...
attr(Golf ,'outlook') <- c("outlook", "Temperatur")
attr(Golf ,'Humidity') <- c("Humidity", "Windy")
str(attributes(Golf)) 
```
```{r}
(SepalVar <- attr(Golf, "outlook")) #Tentukan kelompok variabel berdasarkan atribut
```
```{r}
(PetalVar <- attr(Golf, "Humidity"))
```
```{r}
# Selanjutnya, lihat korelasi dalam menggunakan ggpairs.

ggpairs(Golf  , title = "seluruh data")

```
```{r}
ggpairs(Golf, SepalVar, title = "di dalam outlook ")
```
```{r}
ggpairs(Golf, PetalVar, title = "di dalam Temperatur")
```
