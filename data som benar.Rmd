...
rm(list = ls())
library(readxl)
data_kemiskinan <- read_excel("~/Semester 3/Data Mining/data_kemiskinan.xlsx")
...
str(attributes(data_kemiskinan)) 
```
```{r}
...
ggpairs(data_kemiskinan  , title = "seluruh data")

```
```{r}
ggpairs(data_kemiskinan, SepalVar, title = "di dalam Kemiskinan ")
```
```{r}
ggpairs(data_kemiskinan, PetalVar, title = "di dalam IPM")
```
```{r warning=FALSE}
