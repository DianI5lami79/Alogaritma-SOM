...
library(readxl)
data_som <- read_excel("~/SOM/data som.xlsx")
View(data_som)
```
```{r}
...
text(data_somlatih,labels=data_som$Name,cex=2)
```
```{r}
...
plot.default(data_somlatih,cex=0.09)
text(data_somlatih,labels=data_som$Name,cex=4)
```
```{r}
...
griddata_som<- somgrid(xdim = 2, ydim = 2,topo = "hexagonal" )
```
```{r}
...
'map(data_som)<- som (data_somlatih,grid= griddata_som)'
```
```{r}
